...
func findAssetForPlatform(release *GitHubRelease, goos, goarch string) *GitHubAsset {
	if goos == "darwin" {
		for _, preferred := range []string{"WAtchful-macOS-Universal.zip", "WhatsApp-macOS-Universal.zip"} {
		...
		for _, a := range release.Assets {
			if strings.EqualFold(a.Name, "WAtchful.exe") {
				return &a
			}
		}
		for _, a := range release.Assets {
		...
		for _, preferred := range []string{
			"WAtchful-Linux-" + bundleArch + ".tar.gz",
			// Pre-rename releases still carry the old asset names; keep them
			// selectable (arch-qualified, so the arm64/x64 guard above holds).
...
func updateAssetForPlatform(goos, goarch string) string {
	switch goos {
	case "darwin":
		return "https://github.com/" + githubRepo + "/releases/latest/download/WAtchful-macOS-Universal.zip"
	case "windows":
		return "https://github.com/" + githubRepo + "/releases/latest/download/WAtchful.exe"
	case "linux":
		if goarch == "arm64" || goarch == "aarch64" {
			return "https://github.com/" + githubRepo + "/releases/latest/download/WAtchful-Linux-arm64.tar.gz"
		}
		return "https://github.com/" + githubRepo + "/releases/latest/download/WAtchful-Linux-x64.tar.gz"
	}
	return ""
}
...
// e.g. "WAtchful-macOS-Universal.zip" names carry no version, so the
...
func fetchLatestVersionViaRedirect() (string, error) {
	...
	req.Header.Set("User-Agent", "WAtchful-Updater")
...
set SRC=%s
set DST=%s
set UPDATE_LOG=%%TEMP%%\WAtchful-update.log
...
:copy_failed
> "%%UPDATE_LOG%%" echo WAtchful update could not replace "%%DST%%". The existing version was restarted.
start "" "%%DST%%"
goto cleanup
