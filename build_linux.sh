...
VERSION="${1:-${WA_DESK_VERSION:-2.0.0}}"
...
Version=1.0
Name=${DISPLAY_NAME}
Comment=Lightweight WAtchfultop Client
Exec=${APP_NAME}
Icon=${APP_NAME}
Terminal=false
...
echo "Creating release tarball (WAtchful-Linux-${BUNDLE_ARCH}.tar.gz)..."
TAR_BUNDLE="WAtchful-Linux-${BUNDLE_ARCH}.tar.gz"
rm -f "${TAR_BUNDLE}"
...
Section: net
Priority: optional
Description: Lightweight WAtchfultop Client
 Independent, fast, and privacy-focused WhatsApp desktop application
 built with Go and native WebKit.
EOF

    dpkg-deb --build "${DEB_DIR}" "watchful_${VERSION}_${DEB_ARCH}.deb"
    cp "watchful_${VERSION}_${DEB_ARCH}.deb" "WAtchful-Linux-${DEB_ARCH}.deb"
    rm -rf "${DEB_DIR}"
    echo "Created: watchful_${VERSION}_${DEB_ARCH}.deb and WAtchful-Linux-${DEB_ARCH}.deb"
fi
...
EOF
    ...
    cp "${RPM_TOPDIR}/RPMS/${RPM_ARCH}/${APP_NAME}-${VERSION}-1"*.rpm "WAtchful-Fedora-${BUNDLE_ARCH}.rpm"
    rm -rf "${RPM_TOPDIR}"
    echo "Created: WAtchful-Fedora-${BUNDLE_ARCH}.rpm"
fi
