...
APP_NAME="WAtchful"
VERSION="${1:-2.0.0}"  # single source of truth: injected via -X main.appVersion below
...
if command -v codesign >/dev/null 2>&1; then
    if [ -f "WAtchful.entitlements" ]; then
        codesign --force --deep -s - --options runtime --entitlements WAtchful.entitlements "${BUNDLE_DIR}"
    else
...
if command -v hdiutil >/dev/null 2>&1; then
    echo "Creating DMG installer (WAtchful-macOS-Universal.dmg)..."
    DMG_DIR="dmg_temp"
    rm -rf "${DMG_DIR}" "WAtchful-macOS-Universal.dmg" "WhatsApp-macOS-Universal.dmg"
    mkdir -p "${DMG_DIR}"
    ...
    if hdiutil create -volname "WAtchful" -srcfolder "${DMG_DIR}" -ov -format UDZO "WAtchful-macOS-Universal.dmg" 2>/dev/null; then
        cp "WAtchful-macOS-Universal.dmg" "WhatsApp-macOS-Universal.dmg"
...
echo "Creating ZIP bundle (WAtchful-macOS-Universal.zip)..."
rm -f "WAtchful-macOS-Universal.zip" "WhatsApp-macOS-Universal.zip"
zip -r -y -q "WAtchful-macOS-Universal.zip" "${BUNDLE_DIR}"
cp "WAtchful-macOS-Universal.zip" "WhatsApp-macOS-Universal.zip"
