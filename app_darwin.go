...
static const long long kWAtchfulCacheCapBytes = 512LL * 1024 * 1024;
...
static void enforceDiskCacheCap(void) {
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        @autoreleasepool {
            ...
            if (bytes <= kWAtchfulCacheCapBytes) return;
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (wv) {
            ...
            [wv evaluateJavaScript:wrapped completionHandler:^(id result, NSError *error) {
                if (error) {
                    NSLog(@"[WAtchful] JS evaluation error: %@ for script: %@", error, script);
                }
            }];
        } else {
            NSLog(@"[WAtchful] Error: WKWebView not found for script: %@", script);
        }
    });
}
...
static void openDownloadsFolderNative(void) {
    @autoreleasepool {
        ...
        // Prefer the legacy folder (getSettingsBaseDir keeps pre-rename data
        // in place), then fall back to the new name.
        ...
        if (![[NSFileManager defaultManager] fileExistsAtPath:settingsFile]) {
            settingsFile = [support stringByAppendingPathComponent:@"WAtchful/settings.json"];
        }
        if ([[NSFileManager defaultManager] fileExistsAtPath:settingsFile]) {
...
static NSArray<NSString*>* profileDisplayNames(void) {
    ...
    if (g_profileMenuNames.length > 0) {
        for (NSString* part in [g_profileMenuNames componentsSeparatedByString:@"\n"]) {
            if (part.length > 0) [names addObject:part];
        }
    }
    return names;
}

// addProfilesSubmenu appends a "Profiles" submenu listing every account.
// The active profile is checkmarked and disabled; picking another entry
// relaunches the app into that profile via the Go export.
static void addProfilesSubmenu(NSMenu* parentMenu) {
    NSMenu* profilesMenu = [[NSMenu alloc] initWithTitle:@"Profiles"];
    for (NSString* name in profileDisplayNames()) {
        NSMenuItem* item = [profilesMenu addItemWithTitle:name action:@selector(menuSwitchProfile:) keyEquivalent:@""];
        [item setTarget:g_menuBridge];
        if ([name isEqualToString:g_activeProfileName]) {
            [item setState:NSControlStateValueOn];
            [item setEnabled:NO];
        }
    }
    NSMenuItem* container = [[NSMenuItem alloc] initWithTitle:@"Profiles" action:nil keyEquivalent:@""];
    [container setSubmenu:profilesMenu];
    [parentMenu addItem:container];
}
...
- (void)menuSwitchProfile:(id)sender;
@end
...
- (void)menuSwitchProfile:(id)sender {
    ...
    if (name.length == 0 || [name isEqualToString:g_activeProfileName]) return;
    const char* utf8 = name.UTF8String;
    if (!utf8) return;
    // Go export; relaunches the app into the chosen profile and exits.
    WAtchfulSwitchProfile((char*)utf8);
}
@end
...
// (newline-joined) and the active one. Must be called before
// setupMacOSMenuBar/setupStatusItem; strings are copied synchronously.
static void set_profile_menu_state(const char* names, const char* active) {
    @autoreleasepool {
        g_profileMenuNames = names ? [NSString stringWithUTF8String:names] : @"";
        g_activeProfileName = active ? [NSString stringWithUTF8String:active] : @"";
    }
}
...
static void setupStatusItem(void) {
    @autoreleasepool {
        ...
        if (button) {
            ...
            [button setToolTip:@"WAtchful"];
        }

        NSMenu* trayMenu = [[NSMenu alloc] initWithTitle:@"WhatsApp Tray"];

        NSMenuItem* appTitle = [trayMenu addItemWithTitle:@"WAtchful" action:nil keyEquivalent:@""];
        ...
        // Multi-profile: quick account switcher in the tray menu.
        [trayMenu addItem:[NSMenuItem separatorItem]];
        addProfilesSubmenu(trayMenu);
        ...
        [trayMenu addItemWithTitle:@"Quit WAtchful" action:@selector(terminate:) keyEquivalent:@"q"];
...
static void setupMacOSMenuBar(void) {
    @autoreleasepool {
        ...
        NSMenuItem* aboutItem = [appMenu addItemWithTitle:@"About WAtchful" action:@selector(orderFrontStandardAboutPanel:) keyEquivalent:@""];
        ...
        // Multi-profile: quick account switcher in the menu bar.
        addProfilesSubmenu(appMenu);

        [appMenu addItem:[NSMenuItem separatorItem]];
        [appMenu addItemWithTitle:@"Hide WAtchful" action:@selector(hide:) keyEquivalent:@"h"];
        ...
        [appMenu addItem:[NSMenuItem separatorItem]];
        [appMenu addItemWithTitle:@"Quit WAtchful" action:@selector(terminate:) keyEquivalent:@"q"];
...
func getUserDataDir() string {
	...
	// The WebKit session itself is isolated by the WKWebsiteDataStore in the
	// webview engine, keyed by profile, so accounts never log each other out.
	dir := filepath.Join(appBaseDir(), "Profiles", getActiveProfile().ID+".support")
...
func checkSingleInstance() (*os.File, bool) {
	// Multi-profile: lock per profile (inside the profile's own dir) so two
	// different accounts can run side by side.
	...
	if err != nil {
		_ = exec.Command("osascript", "-e", `tell application "WAtchful" to activate`).Run()
		return nil, false
	}
	return file, true
}
...
func getAppBundlePath() string {
	...
	if err != nil {
		return "/Applications/WAtchful.app"
	}
	if idx := strings.Index(execPath, ".app"); idx != -1 {
		return execPath[:idx+4]
	}
	if _, err := os.Stat("/Applications/WAtchful.app"); err == nil {
		return "/Applications/WAtchful.app"
	}
	return execPath
}
...
func runApp() {
	...
	// Multi-profile: hand the webview engine the active profile id BEFORE the
	// WKWebView is created, so it can attach a per-profile WKWebsiteDataStore.
	if id := getActiveProfile().ID; id != defaultProfileID {
		cid := C.CString(id)
		C.webview_set_profile_id(cid)
	...
	// Multi-profile: seed the Profiles submenu state before the menus are
	// built (both the tray status item and the menu bar reuse it).
	joined, activeName := profileNamesForMenu()
	cj := C.CString(joined)
	ca := C.CString(activeName)
	C.set_profile_menu_state(cj, ca)
	C.free(unsafe.Pointer(cj))
	C.free(unsafe.Pointer(ca))
	C.setupMacOSMenuBar()
	C.setupStatusItem()

	w.SetTitle(windowTitleForProfile())
