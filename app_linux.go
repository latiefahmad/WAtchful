...
static void setWAtchfulGTKTheme(int mode) {
...
static void setWAtchfulKeepAbove(int enable) {
	GtkWindow* top = NULL;
...
#define TRAY_BUS_NAME "org.kde.StatusNotifierItem-watchful"
...
static GVariant* tray_get_property(GDBusConnection* conn, const gchar* sender, const gchar* object_path, const gchar* interface, const gchar* property, GError** error, gpointer user_data) {
	...
	if (strcmp(interface, "org.kde.StatusNotifierItem") == 0) {
		if (strcmp(property, "Id") == 0) {
			return g_variant_new_string("watchful");
		}
		...
		if (strcmp(property, "ToolTip") == 0) {
			...
			return g_variant_new("(sa(iiay)ss)", "watchful", empty, "WAtchful", tip);
		}
	}
...
static void tray_update_overlay_icon(int count) {
	...
	if (count > 0) {
		g_has_overlay = 1;
		snprintf(g_overlay_icon_name, sizeof(g_overlay_icon_name), "watchful-unread-%d", count);
	} else {
		g_has_overlay = 0;
...
func applyNativeThemeLinux(theme string) {
	...
	C.setWAtchfulGTKTheme(mode)
}

func checkSingleInstance() (*os.File, bool) {
	// Multi-profile: lock per profile so two different accounts can run side
	// by side (same-profile double launch is still blocked).
	dataDir := profileDirFor(getActiveProfile())
...
func getUserDataDir() string {
	// Multi-profile: per-profile config home (window state, icon, lock). The
	// WebKit session data lives under ~/.local/share/watchful/profiles/
	// via the per-profile WebKitWebsiteDataManager in the webview engine.
...
func showNativeNotification(title, message, iconPath string) {
	if iconPath != "" {
		_ = exec.Command("notify-send", "-a", "WAtchful", "-i", iconPath, title, message).Run()
	} else {
		_ = exec.Command("notify-send", "-a", "WAtchful", title, message).Run()
	}
}
...
func toggleAlwaysOnTopLinux() bool {
	...
	C.setWAtchfulKeepAbove(C.int(b2i(isAlwaysOnTopLinux)))
	if !isAlwaysOnTopLinux {
		if path, err := exec.LookPath("wmctrl"); err == nil && path != "" {
			// wmctrl matches on the window title substring, and the actual title
			// is the profile-aware windowTitleForProfile(); matching on the base
			// title keeps this working for every profile.
...
func getAutoStartDesktopPath() string {
	...
	return filepath.Join(home, ".config", "autostart", "watchful.desktop")
}
...
Version=1.0
Name=WAtchful
Comment=Lightweight WAtchful Client
Exec=%s
Icon=watchful
Terminal=false
...
func runApp() {
	...
	if !isSingle {
		fmt.Println("WAtchful is already running.")
		os.Exit(0)
	}
	if lockFile != nil {
	...
	linuxCacheHomes = []string{
		filepath.Join(home, ".cache", "watchful"),
		filepath.Join(home, ".cache", "WAtchful"),
	...
	// WebKitWebView is created, so it can create a per-profile session.
	if id := getActiveProfile().ID; id != defaultProfileID {
		C.webview_set_profile_id(C.CString(id))
	}
